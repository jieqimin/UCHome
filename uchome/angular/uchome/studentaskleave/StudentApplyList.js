tempApp.controller("studentapplyList", ["$http", "$scope", function ($http, $scope) {
    $scope.DeleteApply = function (applyId) {
        var target =$(event.target).parents("tr");
        $http.post("/uchome/studentaskleave/DeleteApply", { applyId: applyId }).success(function(response) {
            if (response.flag) {
                alert("删除成功");
