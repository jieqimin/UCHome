 ...
 * Plugin for $ by Christophe Brisbois
 * http://nanogallery.brisbois.fr
 * 
 * External components:
 *  - $ (http://www.jquery.com)
 * 	- $ Color plugin - is embedded
...
(function( $ ) {
	$.fn.nanoGalleryDemo = function(options) {
		...
		return this.each(function(index) {
			g_save=$(this)[0].outerHTML;
			g_containerDemo =$('<div class="nanoGalleryDemo" style="padding:5px"></div>').replaceAll(this);
			...
			g_containerDemoPanel=$(g_containerDemo).append('<div class="nanoGalleryDemoPanel" style="display:table;border:2px solid #666;background:#555;margin:10px auto;padding:5px;font-size:0.8em;">'+sPanel+'</div>');

			g_containerNew=$(g_save).appendTo(g_containerDemo);
			$(g_containerDemoPanel).find('[name=lHoverEffect1]').val('borderLighter');


			var nanoGALLERY_obj = new nanoGALLERY();
			nanoGALLERY_obj.Initiate(g_containerNew,settings);

			$(g_containerDemoPanel).find('button[name=bGo]').on("click",function(){
				runDemo();
			});
			
			$(g_containerDemoPanel).find('button[name=bPreset1]').on("click",function(){
				setPreset('borderLighter','imageSlideRight','none','overImageOnBottom',true,true,'dark');
			});
			$(g_containerDemoPanel).find('button[name=bPreset2]').on("click",function(){
				setPreset('imageRotateCornerBL*','scale120*','borderLighter','overImageOnBottom',true,true,'dark');
			});
			$(g_containerDemoPanel).find('button[name=bPreset3]').on("click",function(){
				setPreset('imageScale150*','labelSlideUp','none','overImageOnBottom',true,false,'dark');
			});
			$(g_containerDemoPanel).find('button[name=bPreset4]').on("click",function(){
				setPreset('imageInvisible','imageScale150Outside*','none','overImageOnBottom',true,false,'light');
			});
			$(g_containerDemoPanel).find('button[name=bPreset5]').on("click",function(){
		...
		function setPreset(hoverEffect1,hoverEffect2,hoverEffect3,thumbnailLabelPosition,thumbnailLabelDisplay,thumbnailLabelDisplayDescription,backgroundColor) {
			$(g_containerDemoPanel).find('[name=lHoverEffect1]').val(hoverEffect1);
			$(g_containerDemoPanel).find('[name=lHoverEffect2]').val(hoverEffect2);
			$(g_containerDemoPanel).find('[name=lHoverEffect3]').val(hoverEffect3);
			$(g_containerDemoPanel).find('input:radio[name=thumbnailLabelPosition]').val([thumbnailLabelPosition]);
			$(g_containerDemoPanel).find('[name=thumbnailLabelDisplay]').prop('checked',thumbnailLabelDisplay);
			$(g_containerDemoPanel).find('[name=thumbnailLabelDisplayDescription]').prop('checked',thumbnailLabelDisplayDescription);
			$(g_containerDemoPanel).find('input:radio[name=backgroundColor]').val([backgroundColor]);
			runDemo();
		}
		
		function runDemo() {
			settings.thumbnailHoverEffect=[];
			var sTHE=$(g_containerDemoPanel).find('[name=lHoverEffect1] option:selected').text();
			if( sTHE != 'none' && sTHE != '' ) {
				sTHE=sTHE.replace('*', '');
				settings.thumbnailHoverEffect.push({'name':sTHE,'duration':200,'durationBack':200,'easing':'swing','easingBack':'swing'});
			}
			sTHE=$(g_containerDemoPanel).find('[name=lHoverEffect2] option:selected').text();
			if( sTHE != 'none' && sTHE != '' ) {
				sTHE=sTHE.replace('*', '');
				settings.thumbnailHoverEffect.push({'name':sTHE,'duration':200,'durationBack':200,'easing':'swing','easingBack':'swing'});
			}
			sTHE=$(g_containerDemoPanel).find('[name=lHoverEffect3] option:selected').text();
			if( sTHE != 'none' && sTHE != '' ) {
				sTHE=sTHE.replace('*', '');
				settings.thumbnailHoverEffect.push({'name':sTHE,'duration':200,'durationBack':200,'easing':'swing','easingBack':'swing'});
			}
			var tW=+parseInt($(g_containerDemoPanel).find('[name=thumbWidth]').val(),10);
			if( tW >= 10 && tW <= 500) {
				$(g_containerDemoPanel).find('[name=thumbWidth]').val(tW);	
				settings.thumbnailWidth=tW;
			}
			var tH=parseInt($(g_containerDemoPanel).find('[name=thumbHeight]').val(),10);
			if( tH>= 10 && tH <=500) {
				$(g_containerDemoPanel).find('[name=thumbHeight]').val(tH);
				settings.thumbnailHeight=tH;
			}
			//var tMIPL=parseInt($(g_containerDemoPanel).find('[name=thumbMaxItemsPerLine]').val(),10);
			//if( tMIPL>= 0 ) {
			//	$(g_containerDemoPanel).find('[name=thumbMaxItemsPerLine]').val(tMIPL);
			//	settings.maxItemsPerLine=tMIPL;
			//}
			//var tMW=parseInt($(g_containerDemoPanel).find('[name=thumbMaxWidth]').val(),10);
			//if( tMW>= 50 ) {
			//	$(g_containerDemoPanel).find('[name=thumbMaxWidth]').val(tMW);
			//	settings.maxWidth=tMW;
			//}
			settings.thumbnailLabel.position=$(g_containerDemoPanel).find('input[name=thumbnailLabelPosition]:checked',g_containerDemoPanel).val();
			settings.thumbnailLabel.display=$(g_containerDemoPanel).find('[name=thumbnailLabelDisplay]').prop('checked');
			settings.thumbnailLabel.displayDescription=$(g_containerDemoPanel).find('[name=thumbnailLabelDisplayDescription]').prop('checked');
			settings.colorScheme=$(g_containerDemoPanel).find('input[name=colorScheme]:checked',g_containerDemoPanel).val();
			settings.theme=$(g_containerDemoPanel).find('input[name=cssFile]:checked',g_containerDemoPanel).val();
			
			if( $(g_containerDemoPanel).find('input[name=backgroundColor]:checked',g_containerDemoPanel).val() == 'dark' ) {
				$(g_containerDemoPanel).css('background','#222');
			}
			else {
				$(g_containerDemoPanel).css('background','#eee');
			}
			
			$(g_containerNew).animate({opacity: 0,height:0},200).promise().done(function(){
			$(g_containerNew).remove();
				g_containerNew=$(g_save).appendTo(g_containerDemo);
				var nanoGALLERY_obj = new nanoGALLERY();
				nanoGALLERY_obj.Initiate(g_containerNew,settings);
				//$(elt).css('opacity','1');
...
function nanoGALLERY() {
	...
	this.Initiate=function(element, params) {
		...
		if( $.support.transition ) { g_supportTransit=true; }
		...
		$(element).addClass('nanogallery_theme_'+g_options.theme);
		...
		if( g_options.maxWidth > 0 ) { 
			$(g_baseControl).css('maxWidth',+g_options.maxWidth);
			$(g_baseControl).css('margin-left','auto');
			$(g_baseControl).css('margin-right','auto');
		}
	

		if( g_options.displayBreadcrumb == true && ( g_options.kind=='picasa' || g_options.kind=='flickr')) {
			g_containerBreadcrumb =$('<div class="nanoGalleryBreadcrumb"></div>').appendTo(element);
		}
		g_containerParent=$('<div class="nanoGalleryContainerParent"></div>').appendTo(element);
		g_containerThumbnails=$('<div class="nanoGalleryContainer"></div>').appendTo(g_containerParent);
		...
		// thumbnails effects
		// easing : $ supports only 'swing' and 'linear'
		switch( toType(g_options.thumbnailHoverEffect) ) {
			...
			case 'object':
				if( g_options.thumbnailHoverEffect.name != undefined ) {
					g_thumbnailHoverEffect.push($.extend({'duration':200,'durationBack':150,'easing':'swing','easingBack':'swing'},g_options.thumbnailHoverEffect));
				}
				break;
			case 'array':
				for(var i=0; i<g_options.thumbnailHoverEffect.length; i++) {
					if( g_options.thumbnailHoverEffect[i].name != undefined ) {
						g_thumbnailHoverEffect.push($.extend({'duration':200,'durationBack':150,'easing':'swing','easingBack':'swing'},g_options.thumbnailHoverEffect[i]));
					}
				}
				break;
			case 'null':
		...
		for( var i=0; i<g_thumbnailHoverEffect.length; i++) {
			switch(g_thumbnailHoverEffect[i].name ) {
				...
				case 'imageRotateCornerBL':
					if ( !g_supportTransit ) {
						nanoConsoleLog('The parameter "'+g_thumbnailHoverEffect[i].name+'" for "thumbnailHoverEffect" requires the additional $ plugin "Transit".');
					}
					break;
				default:
		...
		switch(g_options.kind) {
			case '':
				...
				if( g_options.items !== undefined && g_options.items !== null ) {
					...
					var elements=$(element).children('a');
	...
	function ProcessItemOption() {
		...
		$.each(g_options.items, function(i,item){
	...
	function ProcessHREF(elements) {
		...
		$.each(elements, function(i,item){
			...
			if( $(item).attr('data-ngthumb') !== undefined && $(item).attr('data-ngthumb').length>0 ) {
				newObj.thumbsrc=g_options.itemsBaseURL+$(item).attr('data-ngthumb');
			}
			else {
				newObj.thumbsrc=g_options.itemsBaseURL+$(item).attr('href');
			}
			newObj.src=g_options.itemsBaseURL+$(item).attr('href');
			newObj.description=$(item).attr('data-ngdesc');
			if( $(item).attr('data-ngdesc') !== undefined && $(item).attr('data-ngdesc').length>0 ) {
				newObj.description=$(item).attr('data-ngdesc');
			}
			else {
				newObj.description='';
			}
			if( $(item).attr('data-ngdest') !== undefined && $(item).attr('data-ngdest').length>0 ) {
				newObj.destURL=$(item).attr('data-ngdest');
			}
			else {
				newObj.destURL='';
			}
	...
	function FlickrGetItems( itemID, albumLabel ) {
		...
		if( g_options.displayBreadcrumb == true ) {
			if( albumLabel != "" ) {
				if( $(g_containerBreadcrumb).children().length > 0 ) {
					$('<div class="separator"></div>').appendTo(g_containerBreadcrumb);
				}
				var newDiv =$('<div class="folder">'+albumLabel+'</div>').appendTo(g_containerBreadcrumb);
				$(newDiv).data("path",itemID);
		...
		//$(g_containerThumbnails).children().remove();
	...
	function renderGallery() {
		...
		$(g_containerThumbnails).animate({'max-height': '0'}, 200);
	...
	function renderGallery2() {
		...
		if( g_options.thumbnailLabel.position == 'onBottom'  ) {
			$.each(g_itemInfo, function(i,item){
				if( item.description.length > 0 ) { foundDesc=true; }
			});
		}

		$.each(g_itemInfo, function (i, item) {
		    var name = item['thumbsrc'].substring(22);
		    var lastIndex = name.lastIndexOf('.');
		    var imageNames = "'" + name.substring(0, lastIndex) + "'";
		    var ext = "'" + name.substring(lastIndex) + "'";
			var newDivTemp =$('<div class="container" style="display: inline-block"></div>');	
			var newDivTemp1 =$('<div class="subcontainer" style="display: inline-block"></div>');	
			//$(newDivTemp1).append('<div class="imgContainer" style="width:'+g_options.thumbnailWidth+'px;height:'+g_options.thumbnailHeight+'px;"><img class="image" src="'+item['thumbsrc']+'" style="max-width:'+g_options.thumbnailWidth+'px;max-height:'+g_options.thumbnailHeight+'px;"></div>');
			var newImg = jQuery('<div class="imgContainer" style="width:' + g_options.thumbnailWidth + 'px;height:' + g_options.thumbnailHeight + 'px;">' +
                '<span class="file_del" data-index="0" title="Delete" onclick="chearImage(this,' + imageNames + ',' + ext + ');"></span><img class="image" src="' + item['thumbsrc'] + '" style="max-width:' + g_options.thumbnailWidth + 'px;max-height:' + g_options.thumbnailHeight + 'px;"></div>');
			$(newDivTemp1).append(newImg);
			if( item['kind'] == 'album' ) {
				// album
				if( g_options.thumbnailLabel.display == true ) {
					...
					//var newLabel=$('<div class="labelImage" style="width:'+g_options.thumbnailWidth+'px;max-height:'+g_options.thumbnailHeight+'px;"><div class="labelFolderTitle">'+item.title+'</div><div class="labelDescription">'+sDesc+'</div></div>');
					//$(newDivTemp1).append(newLabel);
				}
			}
			else {
				// image
				...
				if( g_options.thumbnailLabel.display == true ) {
					...
					//var newLabel=$('<div class="labelImage" style="width:'+g_options.thumbnailWidth+'px;max-height:'+g_options.thumbnailHeight+'px;"><div class="labelImageTitle">'+s+'</div><div class="labelDescription">'+sDesc+'</div></div>');
					//$(newDivTemp1).append(newLabel);
				}
			}
			$(newDivTemp).append(newDivTemp1);	
			$(newDivTemp).css('opacity','0');
			var newDiv =$(newDivTemp).appendTo(g_containerThumbnails); //.animate({ opacity: 1},1000, 'swing');	//.show('slow'); //.fadeIn('slow').slideDown('slow');
			...
			switch( g_options.thumbnailLabel.position ){
				case 'onBottom':
					...
					$(newDiv).find('.labelImage').css('position','relative');
			...
			if( g_oneThumbnailWidth == 0 ) {
				//g_oneThumbnailWidth= ($(newDiv).outerWidth(false)-$(newDiv).width()) + $(newDiv).find('.imgContainer').outerWidth(true) ;
				g_oneThumbnailWidth= $(newDiv).find('.imgContainer').outerWidth(true) ;
				//g_oneThumbnailWidth=$(newDiv).outerWidth(true);
			}
			$(newDiv).width(g_oneThumbnailWidth);
			...
			if( g_oneThumbnailHeight == 0 ) {
				...
				//g_oneThumbnailHeight=$(newDiv).outerHeight(true);
			}
			$(newDiv).height(g_oneThumbnailHeight);
		...
		//$(g_containerThumbnails).find('.container').width(200);
		
		$(g_containerThumbnails).animate({maxHeight: newH}, 200, function() {
			// animation to display the thumbnails
			$(g_containerThumbnails).find('.container').each(function(i) {
			...
			switch(g_thumbnailHoverEffect[j].name ) {
				case 'scaleLabelOverImage':
					if( g_options.thumbnailLabel.position == 'overImageOnTop' || g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
						...
						setElementOnTop('', $(elt).find('.imgContainer'));
						$(elt).find('.labelImage').css({'opacity':'0', scale:0.5});
						$(elt).find('.imgContainer').css({ 'scale':'1'});
					}
					break;
				case 'overScale':
					if( g_options.thumbnailLabel.position == 'overImageOnTop' || g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
						$(elt).css('overflow','hidden');
						setElementOnTop('', $(elt).find('.labelImage'));
						$(elt).find('.labelImage').css({'opacity':'0', scale:1.5});
						$(elt).find('.imgContainer').css({ 'opacity': '1', 'scale':'1'});
					}
					break;
				case 'overScaleOutside':
					if( g_options.thumbnailLabel.position == 'overImageOnTop' || g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
				...
				case 'rotateCornerBL':
					if( g_options.thumbnailLabel.position == 'overImageOnTop' || g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
						if( g_supportTransit ) {
							$(elt).css('overflow','hidden');
							$(elt).find('.labelImage').css('opacity','1').css({ rotate: '-90deg', 'transform-origin': '100% 100%' });
							//$(elt).find('.imgContainer').css({ rotate: '0', 'transform-origin': '100% 100%' });
				...
				case 'rotateCornerBR':
					if( g_options.thumbnailLabel.position == 'overImageOnTop' || g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
						if( g_supportTransit ) {
							$(elt).css('overflow','hidden');
							$(elt).find('.labelImage').css('opacity','1').css({ rotate: '90deg', 'transform-origin': '0% 100%' });
							//$(elt).find('.imgContainer').css({ rotate: '0', 'transform-origin': '0 100%' });
							useTransitPlugin=true;
						}
					}
					break;
				case 'imageRotateCornerBL':
					if( g_options.thumbnailLabel.position == 'overImageOnTop' || g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
						if( g_supportTransit ) {
							setElementOnTop(elt, $(elt).find('.imgContainer'));
							$(elt).css('overflow','hidden');
							$(elt).find('.labelImage').css('opacity','1');
							//$(elt).find('.imgContainer').css({ rotate: '0', 'transform-origin': '100% 100%' });
							useTransitPlugin=true;
						}
					}
				...
				case 'slideUp':
					if( g_options.thumbnailLabel.position == 'overImageOnTop' || g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
						$(elt).css('overflow','hidden');
						$(elt).find('.labelImage').css('opacity','1').css('top',g_oneThumbnailHeight);
						$(elt).find('.imgContainer').css({'left':'0', 'top':'0'});
					}
					break;
				case 'slideDown':
					if( g_options.thumbnailLabel.position == 'overImageOnTop' || g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
				...
				case 'slideRight':
					if( g_options.thumbnailLabel.position == 'overImageOnTop' || g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
						$(elt).css('overflow','hidden');
						$(elt).find('.labelImage').css('opacity','1').css('left',-g_oneThumbnailWidth);
						$(elt).find('.imgContainer').css({'left':'0', 'top':'0'});
					}
					break;
				case 'slideLeft':
				...
				case 'labelAppear75':
					if( g_options.thumbnailLabel.position == 'overImageOnTop' || g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
						$(elt).find('.labelImage').css('opacity','0');
					}
					break;
				case 'labelSlideUp':
					if( g_options.thumbnailLabel.position == 'overImageOnTop' || g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
						$(elt).css('overflow','hidden');
						$(elt).find('.labelImage').css({'bottom':-g_oneThumbnailHeight,'top':'none'});
					}
					break;
				case 'labelSlideDown':
					if( g_options.thumbnailLabel.position == 'overImageOnTop' || g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
						$(elt).css('overflow','hidden');
						$(elt).find('.labelImage').css({'top':-g_oneThumbnailHeight, 'bottom':'none'});
					}
					break;
				case 'descriptionSlideUp':
					if( g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
						$(elt).css('overflow','hidden');
						var p=g_oneThumbnailHeight - ($(elt).find('.labelImage').outerHeight(true)-$(elt).find('.labelImage').height()) - g_oneThumbnailLabelTitleHeight +'px';
						$(elt).find('.labelImage').css({'top':p, 'bottom':'none'});
					}
					break;
				...
				case 'imageFlipHorizontal':
					if( g_supportTransit ) {
						...
						//$(elt).css({ perspective: n, 'backface-visibility': 'hidden' });
						//$(elt).find('.subcontainer').css({perspective: n, 'rotateX': '0deg', 'transform-style':'preserve-3d', 'backface-visibility': 'hidden'});
						//$(elt).find('.imgcontainer').css({perspective: n, rotateX: '0deg', 'backface-visibility': 'hidden'});
						$(elt).find('.imgContainer').css({perspective: n, rotateX: '0deg', 'backface-visibility': 'hidden'});
						$(elt).find('.labelImage').css({ perspective: n,rotateX: '180deg', 'backface-visibility': 'hidden','height':g_oneThumbnailHeight,'opacity':'1' });
						useTransitPlugin=true;
					}
					break;
				case 'imageFlipVertical':
					if( g_supportTransit ) {
						...
						//setElementOnTop(elt, $(elt).find('.imgContainer'));
						//$(elt).find('.subcontainer').css({perspective: n, 'rotateY': '0deg', 'transform-style':'preserve-3d', 'backface-visibility': 'hidden'});
						$(elt).find('.imgContainer').css({perspective: n, rotateY: '0deg', 'backface-visibility': 'hidden'});
						$(elt).find('.labelImage').css({ perspective: n, rotateY: '180deg', 'backface-visibility': 'hidden','height':g_oneThumbnailHeight,'opacity':'1' });


						//$(elt).find('.labelImage').css({ perspective: n, rotateY: '180deg', 'backface-visibility': 'hidden', 'opacity':'1', 'height':g_oneThumbnailHeight });
						//$(elt).find('.imgContainer').css({ perspective: n, rotateY: '0deg', 'backface-visibility': 'hidden' });
						useTransitPlugin=true;
					}
					break;
				case 'flipHorizontal':
					var n= Math.round(g_oneThumbnailHeight*1.2) + 'px';
					$(elt).find('.labelImage').css({ perspective: n, rotateX: '180deg', 'backface-visibility': 'hidden', 'opacity':'1', 'height':'100%' });
					useTransitPlugin=true;
					break;
				case 'flipVertical':
					var n= Math.round(g_oneThumbnailWidth*1.2) + 'px';
					$(elt).find('.subcontainer').css({ perspective: n, rotateY: '0deg'});
					$(elt).find('.labelImage').css({ perspective: n, rotateY: '180deg', 'backface-visibility': 'hidden', 'opacity':'1', 'height':'100%' });
					useTransitPlugin=true;
					break;
				case 'borderLighter':
				case 'borderDarker':
	...
	//	//if( jQueryMinVersion('1.9') ) { $(elt).find('*').finish(); }
	//	$(elt).find('*').stop(true,true);
	//	try {
	//		for( j=0; j<g_thumbnailHoverEffect.length; j++) {
	//			switch(g_thumbnailHoverEffect[j].name ) {
	//				case 'scaleLabelOverImage':
	//					if( g_options.thumbnailLabel.position == 'overImageOnTop' || g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
	//						if( g_supportTransit ) {
	//							$(elt).find('.labelImage').transition({ 'opacity': '1', 'scale':'1'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//							$(elt).find('.imgContainer').transition({ 'scale':'0.5'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						}
	//						else {
	//							$(elt).find('.labelImage').animate({ 'opacity': '1', 'scale':'1'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//							$(elt).find('.imgContainer').animate({ 'scale':'0.5'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						}
	//					}
	//					break;
	//				case 'overScale':
	...
	//						if( g_supportTransit ) {
	//							$(elt).find('.labelImage').transition({ 'opacity': '1', 'scale':'1'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//							//$(elt).find('.imgContainer').transition({ 'opacity': '0', 'scale':'0.5'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						}
	//						else {
	//							$(elt).find('.labelImage').animate({ 'opacity': '1', 'scale':'1'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//							//$(elt).find('.imgContainer').animate({ 'opacity': '0', 'scale':'0.5'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						}
	//					}
	//					break;
	//				case 'imageInvisible':
	//						if( g_supportTransit ) {
	//						$(elt).find('.imgContainer').transition({ 'opacity': '0'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//					}
	//					else {
	//						//$(elt).find('.imgContainer').animate({ 'opacity': '0'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//					}
	//					break;
	//				case 'rotateCornerBL':
	//					if( g_supportTransit ) {
	//						if( g_options.thumbnailLabel.position == 'overImageOnTop' || g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
	//							$(elt).find('.labelImage').transition({ rotate: '0deg'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//							$(elt).find('.imgContainer').transition({ rotate: '90deg'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						}
	//					}
	//					break;
	//				case 'rotateCornerBR':
	...
	//						if( g_supportTransit ) {
	//							$(elt).find('.imgContainer').transition({ 'top': n},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//							$(elt).find('.labelImage').transition({ 'top': 0},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						}
	//						else {
	//							$(elt).find('.imgContainer').animate({ 'top': n},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//							$(elt).find('.labelImage').animate({ 'top': 0},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						}
	//					}
	//					break;
	//				case 'slideDown':
	...
	//						if( g_supportTransit ) {
	//							$(elt).find('.imgContainer').transition({ 'left': n},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//							$(elt).find('.labelImage').transition({ 'left': 0},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						}
	//						else {
	//							$(elt).find('.imgContainer').animate({ 'left': n},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//							$(elt).find('.labelImage').animate({ 'left': 0},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						}
	//					}
	//					break;
	//				case 'slideLeft':
	...
	//					if( g_options.thumbnailLabel.position == 'overImageOnTop' || g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
	//						if( g_supportTransit ) {
	//							$(elt).find('.labelImage').transition({ 'opacity': '1'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						}
	//						else {
	//							$(elt).find('.labelImage').animate({ 'opacity': '1'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						}
	//					}
	//					break;
	//				case 'labelAppear75':
	//					if( g_options.thumbnailLabel.position == 'overImageOnTop' || g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
	//						if( g_supportTransit ) {
	//							$(elt).find('.labelImage').transition({ 'opacity': '0.75'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						}
	//						else {
	//							$(elt).find('.labelImage').animate({ 'opacity': '0.75'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						}
	//					}
	//					break;
	//				case 'labelSlideDown':
	...
	//					if( g_options.thumbnailLabel.position == 'overImageOnTop' || g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
	//						if( g_supportTransit ) {
	//							$(elt).find('.labelImage').transition({ 'bottom': 0},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						}
	//						else {
	//							$(elt).find('.labelImage').animate({ 'bottom': 0},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						}
	//					}
	//					break;
	//				case 'descriptionSlideUp':
	...
	//				case 'labelOpacity50':
	//					if( g_supportTransit ) {
	//						$(elt).find('.labelImage').transition({ 'opacity': 0.5 },g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//					}
	//					else {
	//						$(elt).find('.labelImage').animate({ 'opacity': 0.5 },g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//					}
	//					break;
	//				case 'imageOpacity50':
	//					if( g_supportTransit ) {
	//						$(elt).find('.imgContainer').transition({ 'opacity': 0.5 },g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//					}
	//					else {
	//						$(elt).find('.imgContainer').animate({ 'opacity': 0.5 },g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//					}
	//					break;
	//				case 'borderLighter':
	//					if( g_supportTransit ) {
	//						$(elt).transition({ 'borderColor': lighterColor(g_oldBorderColor,0.5) },g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//					}
	//					else {
	//						//$(elt).animate({ 'borderColor': lighterColor(g_oldBorderColor,0.5) },g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						$(elt).animate({ 'borderTopColor': lighterColor(g_oldBorderColor,0.5), 'borderRightColor': lighterColor(g_oldBorderColor,0.5), 'borderBottomColor': lighterColor(g_oldBorderColor,0.5), 'borderLeftColor': lighterColor(g_oldBorderColor,0.5) },g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//					}
	//					break;
	//				case 'borderDarker':
	//					if( g_supportTransit ) {
	//						$(elt).transition({ 'borderColor': darkerColor(g_oldBorderColor,0.5) },g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//					}
	//					else {
	//						//$(elt).animate({ 'borderColor': darkerColor(g_oldBorderColor,0.5) },g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						$(elt).animate({ 'borderTopColor': darkerColor(g_oldBorderColor,0.5), 'borderRightColor': darkerColor(g_oldBorderColor,0.5), 'borderBottomColor': darkerColor(g_oldBorderColor,0.5), 'borderLeftColor': darkerColor(g_oldBorderColor,0.5) },g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//					}
	//					break;
	//				case 'imageScale150':
	//					if( g_supportTransit ) {
	//						$(elt).find('img').transition({ scale: 1.5 },g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//					}
	//					else {
	//						$(elt).find('img').animate({ scale: 1.5 },g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//					}
	//					break;
	//				case 'imageScale150Outside':
	//					setElementOnTop('', elt);
	...
	//					setElementOnTop('', elt);
	//					if( g_supportTransit ) {
	//						$(elt).transition({ scale: 1.2 },g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//					}
	//					else {
	//						$(elt).animate({ scale: 1.2 },g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//					}
	//					break;
	//				case 'imageFlipHorizontal':
	//					if( g_supportTransit ) {
	//						setElementOnTop('', elt);
	//						var n= Math.round(g_oneThumbnailHeight*1.2) + 'px';
	//						//$(elt).find('.subcontainer').transition({ rotateX: '180deg'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						$(elt).find('.imgContainer').transition({ perspective: n, rotateX: '180deg'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						$(elt).find('.labelImage').transition({ perspective: n, rotateX: '360deg'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//					}
	//					break;
	//				case 'imageFlipVertical':
	//					if( g_supportTransit ) {
	//						setElementOnTop('', elt);
	//						var n= Math.round(g_oneThumbnailWidth*1.2) + 'px';
	//						$(elt).find('.imgContainer').transition({ perspective: n, rotateY: '180deg'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						$(elt).find('.labelImage').transition({ perspective: n, rotateY: '360deg'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						//$(elt).find('.subcontainer').transition({ rotateY: '180deg'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//					}
	//					break;
	//				case 'flipHorizontal':
	//					if( g_supportTransit ) {
	//						setElementOnTop('', elt);
	//						var n= Math.round(g_oneThumbnailHeight*1.2) + 'px';
	//						$(elt).transition({ perspective: n, rotateX: '180deg'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//					}
	//					break;
	//				case 'flipVertical':
	//					if( g_supportTransit ) {
	//						setElementOnTop('', elt);
	//						var n= Math.round(g_oneThumbnailWidth*1.2) + 'px';
	//						$(elt).transition({ perspective: n, rotateY: '180deg'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//					}
	//					break;
	//				case 'TEST':
	//					//$(elt).find('img').stop(true, true);
	//					$(elt).find('.subcontainer').transition({ scale: 0.8 },150).transition({ perspective: '50px', rotateX: '180deg'}, 300, 'ease').transition({ scale: 1 },150);
	//					break;
	//			}
	//		}
	//	}
	...
	//					if( g_options.thumbnailLabel.position == 'overImageOnTop' || g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
	//						if( g_supportTransit ) {
	//							$(elt).find('.labelImage').transition({ 'opacity': '0', 'scale':'0.5'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//							$(elt).find('.imgContainer').transition({ 'scale':'1'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//						}
	//						else {
	//							$(elt).find('.labelImage').animate({ 'opacity': '0', 'scale':'0.5'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//							$(elt).find('.imgContainer').animate({ 'scale':'1'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//						}
	//					}
	//					break;
	//				case 'overScale':
	...
	//					if( g_options.thumbnailLabel.position == 'overImageOnTop' || g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
	//					if( g_supportTransit ) {
	//							$(elt).find('.labelImage').transition({ 'opacity': '0', 'scale':'1.5'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//							$(elt).find('.imgContainer').transition({ 'opacity': '1', 'scale':'1'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//						}
	//						else {
	//							$(elt).find('.labelImage').animate({ 'opacity': '0', 'scale':'1.5'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//							$(elt).find('.imgContainer').animate({ 'opacity': '1', 'scale':'1'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//						}
	//					}
	//					break;
	//				case 'imageInvisible':
	//					if( g_supportTransit ) {
	//						$(elt).find('.imgContainer').transition({ 'opacity': '1'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//					}
	//					else {
	//						$(elt).find('.imgContainer').animate({ 'opacity': '1'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//					}
	//					break;
	//				case 'rotateCornerBL':
	//					if( g_supportTransit ) {
	//						if( g_options.thumbnailLabel.position == 'overImageOnTop' || g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
	//							$(elt).find('.labelImage').transition({ rotate: '-90deg'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//							$(elt).find('.imgContainer').transition({ rotate: '0deg'},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						}
	//					}
	//					break;
	//				case 'rotateCornerBR':
	...
	//					if( g_supportTransit ) {
	//						if( g_options.thumbnailLabel.position == 'overImageOnTop' || g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
	//							$(elt).find('.imgContainer').transition({ 'rotate': '0'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack );
	//						}
	//					}
	//					break;
	//				case 'slideUp':
	...
	//						var n= g_oneThumbnailHeight + 'px';
	//						if( g_supportTransit ) {
	//							$(elt).find('.imgContainer').transition({ 'top': 0},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//							$(elt).find('.labelImage').transition({ 'top': n},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						}
	//						else {
	//							$(elt).find('.imgContainer').animate({ 'top': 0},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//							$(elt).find('.labelImage').animate({ 'top': n},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						}
	//					}
	//					break;
	//				case 'slideDown':
	...
	//						var n= '-'+g_oneThumbnailWidth + 'px';
	//						if( g_supportTransit ) {
	//							$(elt).find('.imgContainer').transition({ 'left': 0},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//							$(elt).find('.labelImage').transition({ 'left': n},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						}
	//						else {
	//							$(elt).find('.imgContainer').animate({ 'left': 0},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//							$(elt).find('.labelImage').animate({ 'left': n},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						}
	//					}
	//					break;
	//				case 'slideLeft':
	...
	//					if( g_options.thumbnailLabel.position == 'overImageOnTop' || g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
	//						if( g_supportTransit ) {
	//							$(elt).find('.imgContainer').transition({ 'top': '0'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//							$(elt).find('.imgContainer').transition({ 'left': '0'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//						}
	//						else {
	//							$(elt).find('.imgContainer').animate({ 'top': '0'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//							$(elt).find('.imgContainer').animate({ 'left': '0'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//						}
	//					}
	//					break;
	//				case 'labelAppear':
	...
	//					if( g_options.thumbnailLabel.position == 'overImageOnTop' || g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
	//						if( g_supportTransit ) {
	//							$(elt).find('.labelImage').transition({ 'opacity': '0'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//						}
	//						else {
	//							$(elt).find('.labelImage').animate({ 'opacity': '0'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//						}
	//					}
	//					break;
	//				case 'labelSlideDown':
	//					if( g_options.thumbnailLabel.position == 'overImageOnTop' || g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
	//						if( g_supportTransit ) {
	//							$(elt).find('.labelImage').transition({ 'top': '-99%'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//						}
	//						else {
	//							$(elt).find('.labelImage').animate({ 'top': '-99%'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//						}
	//					}
	//					break;
	//				case 'labelSlideUp':
	//					if( g_options.thumbnailLabel.position == 'overImageOnTop' || g_options.thumbnailLabel.position == 'overImageOnBottom' ) {
	//						if( g_supportTransit ) {
	//							$(elt).find('.labelImage').transition({ 'bottom': '-99%'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//						}
	//						else {
	//							$(elt).find('.labelImage').animate({ 'bottom': '-99%'},g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//						}
	//					}
	//					break;
	//				case 'descriptionSlideUp':
	...
	//						var p=g_oneThumbnailHeight - ($(elt).find('.labelImage').outerHeight(true)-$(elt).find('.labelImage').height()) - g_oneThumbnailLabelTitleHeight +'px';
	//						if( g_supportTransit ) {
	//							$(elt).find('.labelImage').transition({ 'top': p},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						}
	//						else {
	//							$(elt).find('.labelImage').animate({ 'top': p},g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						}
	//					}
	//					break;
	//				case 'labelOpacity50':
	//					if( g_supportTransit ) {
	//						$(elt).find('.labelImage').transition({ 'opacity': g_oldLabelOpacity },g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//					}
	//					else {
	//						$(elt).find('.labelImage').animate({ 'opacity': g_oldLabelOpacity },g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//					}
	//					break;
	//				case 'imageOpacity50':
	//					if( g_supportTransit ) {
	//						$(elt).find('.imgContainer').transition({ 'opacity': 1 },g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//					}
	//					else {
	//						$(elt).find('.imgContainer').animate({ 'opacity': 1 },g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//					}
	//					break;
	//				case 'borderLighter':
	//				case 'borderDarker':
	//					if( g_supportTransit ) {
	//						$(elt).transition({ 'borderColor': g_oldBorderColor },g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//					}
	//					else {
	//						//$(elt).animate({ 'borderColor': g_oldBorderColor },g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//						$(elt).animate({ 'borderTopColor': g_oldBorderColor, 'borderRightColor': g_oldBorderColor, 'borderBottomColor': g_oldBorderColor, 'borderLeftColor': g_oldBorderColor },g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//					}
	//					break;
	//				case 'imageScale150':
	//				case 'imageScale150Outside':
	//					if( g_supportTransit ) {
	//						$(elt).find('img').transition({ scale: 1 },g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//					}
	//					else {
	//						$(elt).find('img').animate({ scale: 1 },g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//					}
	//					break;
	//				case 'scale120':
	//					if( g_supportTransit ) {
	//						$(elt).transition({ scale: 1 },g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//					}
	//					else {
	//						$(elt).animate({ scale: 1 },g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//					}
	//					break;
	//				case 'imageFlipHorizontal':
	//					if( g_supportTransit ) {
	//						var n= Math.round(g_oneThumbnailHeight*1.2) + 'px';
	//						//$(elt).find('.subcontainer').transition({rotateX: '0deg'}, g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//						$(elt).find('.imgContainer').transition({ perspective: n, rotateX: '0deg'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						$(elt).find('.labelImage').transition({ perspective: n, rotateX: '180deg'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//					}
	//					break;
	//				case 'imageFlipVertical':
	//					if( g_supportTransit ) {
	//						var n= Math.round(g_oneThumbnailWidth*1.2) + 'px';
	//						//$(elt).find('.subcontainer').transition({ rotateY: '0deg'}, g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//						$(elt).find('.imgContainer').transition({ perspective: n, rotateY: '0deg'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//						$(elt).find('.labelImage').transition({ perspective: n, rotateY: '180deg'}, g_thumbnailHoverEffect[j].duration, g_thumbnailHoverEffect[j].easing);
	//					}
	//					break;
	//				case 'flipHorizontal':
	//					if( g_supportTransit ) {
	//						var n= Math.round(g_oneThumbnailHeight*1.2) + 'px';
	//						$(elt).transition({ perspective:n, rotateX: '0deg'}, g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//					}
	//					break;
	//				case 'flipVertical':
	//					if( g_supportTransit ) {
	//						var n= Math.round(g_oneThumbnailWidth*1.2) + 'px';
	//						$(elt).transition({ perspective:n, rotateY: '0deg'}, g_thumbnailHoverEffect[j].durationBack, g_thumbnailHoverEffect[j].easingBack);
	//					}
	//					break;
	//				case 'TEST':
	//					//if( jQueryMinVersion('1.9') ) { $(elt).find('.subcontainer').finish(); }
	//					$(elt).find('.subcontainer').transition({ scale: 0.85 },150).transition({ perspective: '50px', rotateX: '0deg'}, 300, 'ease').transition({ scale: 1 },150);
	//					break;
	//			}
	//		}
	//	}
	...
	function SetTumbnailsContainerHeight() {
		var w=$(g_containerThumbnails).width();
		...
		if( g_containerThumbnailsDisplayed ) { $(g_containerThumbnails).css('max-height',h); }
		return h;
	};
	...
	function SetTumbnailsContainerWidth() {
		...
		if( g_oneThumbnailWidthContainer > 0 ) {
			if( g_options.maxWidth > 0 && g_oneThumbnailWidthContainer > g_options.maxWidth ) { $(g_baseControl).css('maxWidth',g_oneThumbnailWidthContainer); }



			var wcont=$(g_containerParent).width();
			if( g_options.maxItemsPerLine > 0 ) {
				if( g_options.maxItemsPerLine*g_oneThumbnailWidthContainer <= wcont ) {
					$(g_containerThumbnails).css('max-width',g_options.maxItemsPerLine*g_oneThumbnailWidthContainer);
				}
				else {
					var w=parseInt(wcont/g_oneThumbnailWidthContainer);
					$(g_containerThumbnails).css('max-width',w*g_oneThumbnailWidthContainer);
				}
			}
			else {
	...
	function OpenInternalViewer(element) {
		g_viewerCurrentItem=$(element).data("index");
		
		//$(g_containerViewer).children().remove();
		//g_containerViewer=$('<div class="nanoGalleryViewer" style="visibility:hidden"></div>').appendTo(g_baseControl);
		g_containerViewerContainer=$('<div class="nanoGalleryViewerContainer" style="visibility:hidden"></div>').appendTo('body');
		$(g_containerViewerContainer).addClass('nanogallery_theme_'+g_options.theme);
		SetColorSchemeViewer(g_containerViewerContainer);

		g_containerViewer=$('<div class="nanoGalleryViewer" style="visibility:hidden"></div>').appendTo(g_containerViewerContainer);
		...
		g_containerViewerContent=$('<div class="content1">'+sImg+'</div>').appendTo(g_containerViewer);
		g_containerViewerCloseFloating=$('<div class="closeButtonFloating"></div>').appendTo(g_containerViewer);
		//g_containerViewerCloseFloating=$('<div class="closeButtonFloating"></div>').appendTo($(g_containerViewerContent).find('img'));
		//g_containerViewerToolbar=$('<div class="toolbar"><div class="previousButton"></div><div class="nextButton"></div><div class="closeButton"></div><div class="label"><div class="title"></div><div class="description"></div></div>').appendTo(g_containerViewer);
		...
		$(g_containerViewerCloseFloating).on("click",function(){
			CloseInternalViewer();
		});
		
		$(g_containerViewerToolbar).find('.closeButton').on("click",function(){
			CloseInternalViewer();
		});
		...
		$(g_containerViewerToolbar).find('.nextButton').on("click",function(){ DisplayNextImage(); });
		$(g_containerViewerToolbar).find('.previousButton').on("click",function(){ DisplayPreviousImage(); });
		$(g_containerViewerContent).find('.contentAreaNext').on("click",function(e){ e.stopPropagation(); DisplayNextImage(); });
		$(g_containerViewerContent).find('.contentAreaPrevious').on("click",function(e){ e.stopPropagation(); DisplayPreviousImage(); });

		
		$(g_containerViewerContent).on("click",function(e){ e.stopPropagation(); CloseInternalViewer(); });
	...
	function DisplayInternalViewer() {
		...
		$(g_containerViewerContent).find('img:eq('+g_viewerCurrentItem+')').attr('src',g_itemInfo[g_viewerCurrentItem].src);
		if( (g_viewerCurrentItem+1) < g_itemInfo.length ) {
			// set also the url of the next image for pre-loading
			$(g_containerViewerContent).find('img:eq('+(g_viewerCurrentItem+1)+')').attr('src',g_itemInfo[g_viewerCurrentItem+1].src);
		}
		if( g_viewerPreviousItem != -1 ) {
			set2ElementsOnTop( $(g_containerViewerContent), $(g_containerViewerContent).find('img:eq('+g_viewerPreviousItem+')'), $(g_containerViewerContent).find('img:eq('+g_viewerCurrentItem+')') );
		}
		else {
			//set2ElementsOnTop( $(g_containerViewerContent), $(g_containerViewerContent).find('img:eq('+g_viewerPreviousItem+')'), $(g_containerViewerContent).find('img:eq('+g_viewerCurrentItem+')') );
		}
		...
		$(g_containerViewerContent).find('img:eq('+g_viewerCurrentItem+')').css({'visibility':'visible', 'opacity':0});
		if( g_viewerPreviousItem != -1 ) {
			$(g_containerViewerContent).find('img:eq('+g_viewerCurrentItem+')').animate({ 'opacity': '1' });
			$(g_containerViewerContent).find('img:eq('+g_viewerPreviousItem+')').animate({ 'opacity': '0' }, function() {
				$(g_containerViewerContent).find('img:eq('+g_viewerPreviousItem+')').css('visibility','hidden');
		...
		if( g_itemInfo[g_viewerCurrentItem]['title'] !== undefined ) {
			$(g_containerViewerToolbar).find('.title').text(g_itemInfo[g_viewerCurrentItem]['title']);
		}
		else {
			$(g_containerViewerToolbar).find('.title').text('');
		}
		// set description
		if( g_itemInfo[g_viewerCurrentItem]['description'] !== undefined ) {
			$(g_containerViewerToolbar).find('.description').text(g_itemInfo[g_viewerCurrentItem]['description']);
		}
		else {
			$(g_containerViewerToolbar).find('.description').text('');
		}
	...
	function CloseInternalViewer() {
		if( g_containerViewerDisplayed ) {
			g_containerViewerDisplayed=false;
			$(g_containerViewer).remove();
			$(g_containerViewerContainer).remove();
	...
	function ResizeInternalViewer() {
		...
		var elt=$(g_containerViewerContent).find('img');
		...
		var h=$(window).height()-$(g_containerViewerToolbar).outerHeight(true)-contentOuterWidthV;
		var w=$(window).width()-contentOuterWidthH;
		$(g_containerViewerContent).css({'height':h, 'width':w	});
				
		$(g_containerViewerContent).children('img').css({'max-width': (w-tV),'max-height':h-tH });
		
		var pt=($(g_containerViewerContent).find('img').outerHeight(true)-$(g_containerViewerContent).find('img').outerHeight())/2;
		var pl=($(g_containerViewerContent).find('img').outerWidth(true)-$(g_containerViewerContent).find('img').outerWidth())/2;
		//$(g_containerViewerCloseFloating).css('top',pt-11);
		//$(g_containerViewerCloseFloating).css('left',pl-11);
		
	};
	
	
	function OpenFancyBox(element) {
		var n=$(element).data("index");
		...
		$.fancybox.open(lstImages,{'autoPlay':false, 'nextEffect':'fade', 'prevEffect':'fade','scrolling':'no',
	...
	function SetColorScheme(element) {
		...
		switch(toType(g_options.colorScheme)) {
			case 'object':		// user custom color scheme object 
				...
				$.extend(true,cs,g_options.colorScheme);
		...
		$(element).addClass(g_colorSchemeLabel);
	...
	function SetColorSchemeViewer(element) {
		...
		switch(toType(g_options.colorSchemeViewer)) {
			case 'object':		// user custom color scheme object 
				...
				$.extend(true,cs,g_options.colorSchemeViewer);
		...
		s+=s1+'.nanoGalleryViewer .content1 img { border:'+cs.imageBorder+'; box-shadow:'+cs.imageBoxShadow+'; }'+'\n';
	...
	// DEMO PANEL
	//	g_containerDemo =$('<div class="nanoGalleryDemo"></div>').appendTo(element);
	...
	function setElementOnTop( start, elt ) {
		...
		$(start).each(function() {
			...
			// if ($(this).css("z-index") > highest_index) {
				//highest_index = parseInt($(this).attr("z-index"));
			// }
		});
		highest_index++;
		$(elt).css('z-index',highest_index);
	};
	...
	function set2ElementsOnTop( start, elt1, elt2 ) {
		...
		highest_index++;
		$(elt2).css('z-index',highest_index+1);
		$(elt1).css('z-index',highest_index);
	};
	...
	// return true if current $ version match the minimum required
	function jQueryMinVersion(version) {
		var $vrs = window.$.fn.jquery.split('.'),
		min = version.split('.');
