	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			var defaults = {
					...
					url              : "/upload/UploadAction",  	// 上传图片的路径
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
				    ...
				    html += '							<img class="upload_image" src="../Content/images/uploadImage/add_img.png" style="width:expression(this.width > ' + imgWidth + ' ? ' + imgWidth + 'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSuccess: function(file, response) {
						...
						var imageName = response.substring(22);
						var i = imageName.lastIndexOf('.');
					    var imageNames = response.substring(22, 22 + i);

						var html = "<a id=\"" + imageNames + "\" href=\"" + imageName + "\" data-ngthumb=\"" + imageName + "\" data-ngdesc=\"" + file.name + "\"></a>";
					    $("#imageHtml").append(html);
					    $("#uploadedImages").html($("#imageHtml").html());
					    $("#hidImages").val($("#hidImages").val() + imageName + ";");
